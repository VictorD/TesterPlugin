...
public class TesterListener extends PluginListener {	
    
	public TesterListener(TesterPlugin p) {
	}
    ...
    public boolean onCommand(Player player, String[] split) {
        ...
        if (split[0].equalsIgnoreCase("/test")) {
            if (split.length < 2) {
                player.notify("No. Valid syntax is: /test <test name> (optional params)");
            } else {
               TestKeeper.runTest(split[1], split);
            }
            return true;
        }
    ...
    public boolean onComplexBlockChange(Player player, ComplexBlock block) {
        ...
        if(block instanceof ItemArray ) {
            for(Item i : ((ItemArray<?>)block).getContents()) {
                if (i != null)
                    System.out.println("Item: " + i.toString());
            }
        } else if (block instanceof Sign) {
            for(int i = 0; i < 4; i++)
                System.out.println("SIGN SAYS: " + ((Sign)block).getText(i));
            
            new Boat(block.getX()+5, block.getY(), block.getZ());
            new Minecart(block.getX(), block.getY(), block.getZ(), Minecart.Type.Minecart);
            new Minecart(block.getX()+2, block.getY(), block.getZ(), Minecart.Type.PoweredMinecart);
            new Minecart(block.getX()+5, block.getY(), block.getZ()+2, Minecart.Type.StorageCart);
        }
           return false;
       }
     ...
     * @return
     */
    public boolean onDamage(PluginLoader.DamageType damageType, BaseEntity attacker, BaseEntity defender, int amount) {
        String reply = "Unknown";
        switch(damageType) {
        case ENTITY:
            Mob m = new Mob((ka) defender.getEntity());
            System.out.println("Mob: " + m.getName());
            reply = "Damaged by entity";
            break;
        case FIRE:
            reply = "Fire damage"; 
            break;
        ...
        case FALL:
            ...
            break;
        case EXPLOSION:
            reply = "Explosion damage";
            break;
        case CREEPER_EXPLOSION:
            reply = "Creeper Explosion damage";
            break;
        }

        if (defender.isPlayer()) {
            defender.getPlayer().sendMessage(reply + " blocked: " + amount + " and time is " + System.currentTimeMillis());
            return true;
        }
        return false;
    }
    ...
    public boolean onBlockPlace(Player player, Block blockPlaced, Block blockClicked, Item itemInHand) {
        LogSender.send("Block(" + blockPlaced.getType() + ") placed by " + player.getName() + " at " + blockPlaced.getX() + 
